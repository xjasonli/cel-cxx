...
impl<'f> ProgramInner<'f> {
    pub fn new_from_env_source<S: AsRef<[u8]>>(
        ...
        ProgramInnerTryBuilder {
            ...
            ffi_program_builder: |env: &Arc<EnvInner<'f>>, ffi_ctx: &ffi::Ctx| {
                ...
                let ast = result.pin_mut().release_ast()
                    .map_err(|ffi_status| ffi::error_to_rust(&ffi_status))?;
                let return_type = ffi::type_to_rust(&ast.return_type(ffi_ctx.descriptor_pool(), ffi_ctx.arena()));

                let program = env.runtime().create_program(ast)
                    .map_err(|ffi_status| ffi::error_to_rust(&ffi_status))?;
        ...
        A: ActivationInterface<'f, ()> + 'a,
    {
        ...
        let ffi_activation = ffi::Activation::new(
            ffi::FfiActivationImpl::new(
        ...
        let ffi_value = self.ffi_program()
            .evaluate(
                eval_ctx.arena(),
        ...
        Afm: FnMarker,
    {
        ...
        let ffi_activation = ffi::Activation::new(
            ffi::async_activation::FfiAsyncActivationImpl::<Rm::BlockingRunner>::new(
                self.env().function_registry(),
                self.env().variable_registry(),
                activation.functions(),
                activation.variables(),
